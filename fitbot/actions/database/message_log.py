...
def create_user_lead(chat_id, username, nome, email, cpf, telefone, tipo_conta, token):
    try:
        lead_user_model = LeadsModel(
                    token=token,
                    chat_id=chat_id,
                    username=username,
                    nome=nome,
        ...
        print(lead_user_model.username)
        message_log = create(lead_user_model)
        return message_log
    except Exception as e:
        print(e)

def delete_user_lead(cpf):
    try:    
        with DBContextManager() as session:
            lead_user_model = session.query(LeadsModel).filter(LeadsModel.cpf == cpf).first()
            if lead_user_model:
                session.delete(lead_user_model)
                session.commit()
            else:
                return False
    except Exception as e:
...
def query_user_lead(token):
    try:    
        with DBContextManager() as session:
            lead_user_model = session.query(LeadsModel).filter(LeadsModel.token == token).first()
        if lead_user_model:
            return lead_user_model
        else:
            return False
    except Exception as e:
        print(e)

def query_user_account_cpf(cpf):
    try:    
        with DBContextManager() as session:
            account_user_model = session.query(ContasModel).filter(ContasModel.cpf == cpf).first()
        if account_user_model:
            return account_user_model
        else:
            return False
    except Exception as e:
        print(e)

def query_user_account_username(username):
    try:    
        with DBContextManager() as session:
            account_user_model = session.query(ContasModel).filter(ContasModel.username == username).first()
        if account_user_model:
            return account_user_model
        else:
            return False
